    ...
    fn from(value: sqlx::Error) -> Self {
        ...
        match value {
            ...
            TypeNotFound { type_name } => {
                DbError::new(Unprocessable(Owned(format!("Type name {type_name} not found!"))))
            }
            ColumnDecode { index, source } => {
                DbError::new(Unprocessable(Owned(format!("Cloumn Decode Error{index}, {source}"))))
            }
            ...
            InvalidSavePointStatement => {
                DbError::new(Unprocessable(Borrowed("Invalid Save Point Statement (Trigger)")))
            }
