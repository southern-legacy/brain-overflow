...
async fn post_article(
    ...
    Ok((
        StatusCode::OK,
        [(header::LOCATION, format!("/asset/{new_article}"))],
...
#[debug_handler]
async fn get_article_of_someone(State(state): State<ServerState>, Path(id): Path<Uuid>, Query(opts): Query<PageOption>) -> ApiResult {
    let res = Article::by_author(&state.database, id, opts).await?;
    Ok((StatusCode::OK, axum::Json(res)).into_response())
}

#[debug_handler]
async fn get_article_by_title(State(state): State<ServerState>, Path(title): Path<String>, Query(opts): Query<PageOption>) -> ApiResult {
